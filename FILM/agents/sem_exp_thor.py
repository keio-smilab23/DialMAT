...
class Sem_Exp_Env_Agent_Thor(ThorEnvCode):
	...
	def load_initial_scene(self):
		...
		try:
			if self.args.dialfred:
				...
				self.traj_data = traj_data; self.r_idx = 0
				r_idx = 0
				self.r_idx = 0
				...
				high_level_actions = traj_data["high_level_actions"]
				list_of_actions = []
				for data in high_level_actions['list_of_highlevel_actions']:
					list_of_actions.append( (data[0], data[1]) )

				categories_in_inst = high_level_actions['categories_in_inst']
				second_object = high_level_actions['second_object']
				caution_pointers = high_level_actions['caution_pointers']
				task_type = high_level_actions['task_type']
				sliced = high_level_actions['sliced']
				self.output_env_point = high_level_actions['output_env_point']
	...
	def reset_total_cat_new(self, categories_in_inst):
		...
		cat_counter = 0
		assert len(categories_in_inst) <= 10
	...
	def consecutive_interaction(self, interaction, target_instance):
		...
		self.output_submisssion_file_for_dialfred_challenge(interaction)
		# if self.action_number in self.output_env_point:
		# 	print(f"=== {self.action_number} : {interaction} <==")
		# else:	
		# 	print(f"=== {self.action_number} : {interaction}")
		# self.action_number += 1
	...
	def output_submisssion_file_for_dialfred_challenge(self, interaction):
		if self.action_number in self.output_env_point:
			print(f"=== {self.action_number} : {interaction} <==")
			m_data = self.event.metadata
			m_data["pose_discrete"] = self.event.pose_discrete
			self.submission_data.append(m_data)

			with open(f"submission_file/{self.scene_names[self.scene_pointer]}.json", "w") as wf:
				json.dump(self.submission_data, wf, indent=2)
			wf.close()
		else:	
			print(f"=== {self.action_number} : {interaction}")
		self.action_number += 1
	...
	def plan_act_and_preprocess(self, planner_inputs, goal_spotted):
		...
		if self.side_step_order in [1,2]:
			...
			if interaction == "MoveTo":
				# m_data = self.event.metadata
				# m_data["pose_discrete"] = self.event.pose_discrete
				# print("0=== " + interaction)
				# list_of_actions = planner_inputs['list_of_actions']
				# pointer = planner_inputs['list_of_actions_pointer']
				# print(list_of_actions[pointer])
				# meta_data.append(m_data)
				# print(type(self.event.metadata))
				# print(self.event.pose_discrete)
				# with open("output.json", "w") as wf:
				# 	json.dump(sorted(self.event.metadata), wf, indent=2, ensure_ascii=False)
				# wf.close()
				# print(self.event.last_event.metadata.pose_discrete)
				# sys.exit()
				pass
		...
		# if flag == True:
		# new_m_data = self.event.metadata
		# print(new_m_data == m_data)
		# list_of_actions = planner_inputs['list_of_actions']
		# pointer = planner_inputs['list_of_actions_pointer']
		# print(list_of_actions[pointer])
		# sys.exit()
		...
		# if float(self.event.metadata["currentTime"]) > 600:

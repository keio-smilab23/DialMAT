...
def generate_attention_mask(len_lang, len_frames, len_actions,  device, len_subword, num_input_actions=0, is_maskrcnn=False, is_mask=True,  is_clip_resnet=False, is_pallarel=False,  num_of_use=1):
    '''
    ...
    if is_clip_resnet:
        ...
        lang_to_rest = torch.ones((len_lang, len_frames * 3), device=device).float() * float('-inf')
        ...
        frames_clip_to_frames = triangular_mask(len_frames, device)
        frames_resnet_to_frames = triangular_mask(len_frames, device)
        ...
        frames_clip_to_resnet = triangular_mask(len_frames, device)
        frames_resnet_to_clip = triangular_mask(len_frames, device)
        ...
        frames_clip_to_all = torch.cat((frames_clip_to_lang, frames_clip_to_resnet, frames_clip_to_frames, frames_clip_to_actions), dim=1)
        frames_resnet_to_all = torch.cat((frames_resnet_to_lang, frames_resnet_to_frames, frames_resnet_to_clip, frames_resnet_to_actions), dim=1)
        ...
        lang_to_rest = torch.ones((len_lang, len_frames * 2 + len_frames * len_subword), device=device).float() * float('-inf')
        ...
        frames_resnet_to_all = torch.cat((frames_resnet_to_lang, frames_resnet_to_frames_resnet, frames_resnet_to_bbox, frames_resnet_to_actions), dim=1)
        ...
        for a_idx in range(num_input_actions):
            for i in range(len_frames * len_subword):
                f_idx = i // (len_subword)
                if f_idx - 1 - a_idx < 0:
                    # the index is out of bound
                    continue
                bbox_to_actions[i, f_idx - 1 - a_idx] = 0.
        bbox_to_all = torch.cat((bbox_to_lang, bbox_to_frames_resnet, bbox_to_bbox, bbox_to_actions), dim=1)
        ...
        # actions_to_all = frames_to_all.clone()
        # 4. concatenate all the masks
        all_to_all = torch.cat((lang_to_all, frames_resnet_to_all, bbox_to_all, actions_to_all), dim=0)
        ...
        # frames_resnet_to_clip = torch.zeros((len_frames, len_frames), device=device).float()
        # frames_resnet_to_frames_clip = triangular_mask(len_frames, device)
        frames_resnet_to_frames_bbox = torch.ones((len_frames, len_frames * len_subword * num_of_use), device=device).float() * float('-inf')
        # id:204
        # for i in range(len_frames):
        #     frames_resnet_to_frames_bbox[i, :(i+1) * len_subword * num_of_use] = 0.
        frames_resnet_to_frames_label = frames_resnet_to_frames_bbox.clone()
        ...
        frames_resnet_to_all = torch.cat((frames_resnet_to_lang, frames_resnet_to_frames_resnet, frames_resnet_to_frames_clip, frames_resnet_to_frames_bbox, frames_resnet_to_frames_label, frames_resnet_to_actions), dim=1)
        frames_clip_to_all = frames_resnet_to_all.clone()
        # bboxとlabelの次元は(length_frames * length_subgoal * 5)であり、次のフレームの情報をみないように階段状のマスクを作成する(幅がlength_subgoal * 5)
        # frames_bbox_to_lang = torch.ones((len_frames * len_subword * num_of_use, len_lang), device=device).float() * float('-inf')
        #id:204
        ...
        frames_bbox_to_frames_resnet = torch.ones((len_frames * len_subword * num_of_use, len_frames), device=device).float() * float('-inf')
        # frames_bbox_to_frames_resnet = torch.zeros((len_frames * len_subword * num_of_use, len_frames), device=device).float()
        for i in range(0, len_frames * len_subword * num_of_use, len_subword * num_of_use):
            frames_bbox_to_frames_resnet[i:i+len_subword * num_of_use, :(i // (len_subword * num_of_use))+1] = 0.
        
        frames_bbox_to_frames_clip = frames_bbox_to_frames_resnet.clone()
        ...
        frames_bbox_to_all = torch.cat((frames_bbox_to_lang, frames_bbox_to_frames_resnet, frames_bbox_to_frames_clip, frames_bbox_to_frames_bbox, frames_bbox_to_frames_label, frames_bbox_to_actions), dim=1)

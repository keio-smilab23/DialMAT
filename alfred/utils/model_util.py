...
def generate_attention_mask(len_lang, len_frames, len_actions,  device, num_input_actions=0, is_maskrcnn=True,  is_clip_resnet=False):
    '''
    ...
    if is_clip_resnet:
        ...
        all_to_all = torch.cat((lang_to_all, frames_resnet_to_all, frames_clip_to_all, actions_to_all), dim=0)
                  ...
                  # 1. language should attend only to language
        lang_to_lang = torch.zeros((len_lang, len_lang), device=device).float()
        lang_to_rest = torch.ones((len_lang, len_frames * 5), device=device).float() * float('-inf')
        lang_to_all = torch.cat((lang_to_lang, lang_to_rest), dim=1)
        # 2.1 frames should attend to all language tokens
        frames_clip_to_lang = torch.zeros((len_frames, len_lang), device=device).float()
        frames_resnet_to_lang = torch.zeros((len_frames, len_lang), device=device).float()
        ...
        frames_clip_to_all = torch.cat((frames_clip_to_lang, frames_clip_to_resnet, frames_clip_to_frames, frames_clip_to_actions), dim=1)
        frames_resnet_to_all = torch.cat((frames_resnet_to_lang, frames_resnet_to_frames, frames_resnet_to_clip, frames_resnet_to_actions), dim=1)
        frames_bbox_to_all = frames_clip_to_all.clone()
        frames_label_to_all = frames_clip_to_all.clone()
        # 3. actions should attend to the same indices as frames
        # framesとactionsの次元は違う
        actions_to_all = frames_clip_to_all.clone()
        # actions_to_all = frames_to_all.clone()

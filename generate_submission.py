...
def generate_submission(input_json):
    ...
    save_path_full = os.path.join(os.environ['DF_ROOT'] + "/results/submission/" +input_json[-9:])
    with open(save_path_full, "w") as f:
        json.dump(meta_data, f, sort_keys=True, indent=4)   

def main():
    # path to testset json file 
    os.makedirs(os.path.join(os.environ['DF_ROOT'] + "/results/submission/"), exist_ok=True)
    files = glob.glob("./testset/dialfred_testset_final/*")

...
def evalIters(args, lang, dataset, encoder, decoder, critic, performer, extractor, all_ans, split_id, max_steps, print_every=1, save_every=100, current_epoch=0, output_dir=""):
    start = time.time()
    ...
    for dataset_idx in data_instruct_list:
        ...
        for subgoal_idx in subgoal_idxs:
            ...
            if it % save_every == 0:
                if not os.path.exists(os.path.join(output_dir, "questioner_rl")):
                    os.makedirs(os.path.join(output_dir, "questioner_rl"))
                with open(os.path.join(output_dir, "questioner_rl/eval_questioner_anytime_" + split_id + "_epoch" + str(current_epoch) + ".pkl"), "wb") as pkl_f:
    ...
    if args.mode == "valid":
        ...
        with open(os.path.join(output_dir, "result_of_all_epochs.json"), "a") as f:
            #epochをkeyにして、reward, sr, pwsを保存
            json.dump({current_epoch: {"reward": np.mean(all_rewards), "sr": np.mean(succ), "pws": np.mean(all_pws)}}, f)
            f.write("\n")
        ##output_dirの中にdone{:02d}.txtを作成
        with open(os.path.join(output_dir, "done{:02d}.txt".format(current_epoch)), "w") as f:
            f.write("done")
    elif args.mode == "test":
        # save the final result
        with open(os.path.join(output_dir, "test_result_of_best_model.json"), "a") as f:
            #sr, pwsを保存
            json.dump({"sr": np.mean(succ), "pws": np.mean(all_pws)}, f)
            f.write("\n")
            ##output_dirの中にdone{:02d}.txtを作成
...
def evalModel(args):
    ...
    np.random.seed(0)
    data_split = "pseudo_{}".format(args.mode)
    train_id = 1
    logging.basicConfig(filename=os.path.join(args.output_dir,'rl_anytime_eval_'+ data_split + str(train_id) + "_epoch" + str(args.epoch) + '.log'), level=logging.INFO)
    ...
    evalIters(model_args, lang, dataset, encoder, decoder, critic, performer, extractor, all_ans, split_id=data_split + str(train_id), max_steps=1000, print_every=1, save_every=10, current_epoch=current_epoch, output_dir=output_dir)
...
def main():
    ...
    parser.add_argument("--mode", dest="mode", type=str, default="valid")
    parser.add_argument("--epoch", dest="epoch", type=int, default=0)
    parser.add_argument("--output_dir", dest="output_dir", type=str, default="./logs/et_augmented_human_subgoal_splited")
